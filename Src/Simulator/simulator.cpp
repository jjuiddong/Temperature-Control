...
cViewer::cViewer()
{
	m_windowName = L"Temperature Control Simulator";
	//m_isLazyMode = true;
...
bool cViewer::OnInit()
{
	...
	m_inputView = new cInputView();
	m_inputView->Create(eDockState::DOCKWINDOW, eDockSlot::TAB, this, NULL);
	bool result = m_inputView->Init(m_renderer);
	assert(result);

	m_paramView = new cParametersView();
	m_paramView->Create(eDockState::DOCKWINDOW, eDockSlot::RIGHT, this, m_inputView, 0.3f);
	result = m_paramView->Init(m_renderer);
	assert(result);

	m_outputView = new cOutputView();
	m_outputView->Create(eDockState::DOCKWINDOW, eDockSlot::BOTTOM, this, m_inputView, 0.25f);
	result = m_outputView->Init(m_renderer);
	assert(result);
